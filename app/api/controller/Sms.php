...
class Sms
{
	...
	public function send_notice($mobile, $arr = [], $msg_type = '')
	{
		...
		if (!$msg_type) {
			$model_code = config('alidayu.smsType')['login']['templateCode']; // 短信验证码模板code
		} else {
			$model_code = config('alidayu.smsType')[$msg_type]['templateCode']; // 指定模板code
		}
		// $model_code = $model_code ? $model_code : config('alidayu.smsType')['bind']['templateCode']; // 模版code
		$accessKeyId =  config('alidayu.AccessKey'); //参考本文档步骤2
		$accessKeySecret =  config('alidayu.SecretKey'); //参考本文档步骤2
		...
		//必填-短信签名
        if (!$msg_type) {
          $request->setSignName(config('alidayu.smsType')['login']['signName']);
        } else {
          $request->setSignName(config('alidayu.smsType')[$msg_type]['signName']);
        }
