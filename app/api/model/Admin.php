...
class Admin extends BaseModel
{
    ...
    public function setProvinceUuidAttr($value)
    {
      return json_encode($value);
    }

    public function getProvinceUuidAttr($value)
    {
      return json_decode($value);
    }

    public function setAreaUuidAttr($value)
    {
      return json_encode($value);
    }

    public function getAreaUuidAttr($value)
    {
      return json_decode($value);
    }

    public function setCityUuidAttr($value)
    {
      return json_encode($value);
    }

    public function getCityUuidAttr($value)
    {
      return json_decode($value);
    }
    ...
    public function login($mobile, $password)
    {
        ...
        // 用户登陆
        $user = self::field('*')
            ->where(['uname' => $mobile, 'password' => $password, 'is_deleted' => 1])
            ->find();

        // 如果用户不存在，则报错
        if (empty($user)) {
            AdminLog::build()->add($user['uuid'], '账号不存在或者密码错误，登陆失败');
            throw new Exception('账号不存在或者密码错误，登陆失败', 403);
        }
