...
function get_sim($uuid){
  $area_uuid = Admin::build()->where('uuid',$uuid)->value('area_uuid');
  $arr = json_decode($area_uuid);
  if($arr){
    return Device::where('area_uuid','in',$arr)->column('sim');
  }
}
...
function address_to_lat($address){
  ...
  $param['output']   = 'json';
  $param['key']   = 'f113cb55f97a354c8ad7e9e1378f0157';
